...
typedef Callback<void(MessageData &)>  MessageCB;
...
public:
    ...
    void addTopicHandler(const char * topic, MessageCB on_message)
    {
        // FP<void,MessageData &> fp;
        // fp.attach(object, member);

        topicCBMap.insert(std::pair<std::string, MessageCB >(std::string(topic), on_message));  
    }
...
private:
    ...
    std::map<std::string,  MessageCB> topicCBMap;
    
    unsigned char sendbuf[MAX_MQTT_PACKET_SIZE];
    unsigned char readbuf[MAX_MQTT_PACKET_SIZE];

    chrono::milliseconds keepAliveInterval;
    Timer comTimer;
    ...
    int readUntil(int packetType, chrono::milliseconds timeout);
