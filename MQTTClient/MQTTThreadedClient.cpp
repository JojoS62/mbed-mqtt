 ...
 **/
int MQTTThreadedClient::readUntil(int packetType, chrono::milliseconds timeout)
{
    int pType = FAILURE;
    Timer timer;
...
int MQTTThreadedClient::login()
{
    ...
    keepAliveInterval = std::chrono::milliseconds(connect_options.keepAliveInterval * 1000);
...
int MQTTThreadedClient::connect()
{
    ...
    SocketAddress remote(host.c_str(), port);
    if (( ret = tcpSocket->connect(remote)) < 0 )
    {
...
{
#if 0
    int id = queue.call(mbed::callback(this, &MQTTThreadedClient::sendPublish), topic, msg);
    ...
    if (id == 0)
        ...
        return SUCCESS;
#else
    PubMessage *message = mpool.try_alloc();
    if (message) {
        // Simple copy
        *message = msg;
...
void MQTTThreadedClient::addTopicHandler(const char * topicstr, void (*function)(MessageData &))
{
    ...
    Callback<void(MessageData &)> cb;
    cb = function;
    
    topicCBMap.insert(std::pair<std::string, MessageCB >(std::string(topicstr), cb));    
} 
...
int MQTTThreadedClient::processSubscriptions()
{
    ...
    std::map<std::string, MessageCB >::iterator it;
...
bool MQTTThreadedClient::hasConnectionTimedOut()
{
    if (keepAliveInterval > 0ms ) {
        ...
        if (comTimer.elapsed_time() > keepAliveInterval)
            return true;
        else
            return false;
    }
...
void MQTTThreadedClient::startListener()
{
    ...
    while(true)
    {
        ...
        while(true) 
        {
            ...
            PubMessage *message;
            if (mqueue.try_get_for(10ms, &message)) {
